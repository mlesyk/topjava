...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList,
        ...
        for (UserMeal userMeal : mealList) {
            ...
            caloriesSumPerDay.merge(date, userMeal.getCalories(), (value, newValue) -> value + newValue);
        }

        return mealList.stream()
                .filter(userMeal -> TimeUtil.isBetween(userMeal.getDateTime().toLocalTime(), startTime, endTime))
                .map(userMeal -> convertUserMealToExceeded(userMeal,
                        caloriesSumPerDay.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay))
                .collect(Collectors.toList());
    }

    public static UserMealWithExceed convertUserMealToExceeded(UserMeal userMeal, boolean exceeded) {
